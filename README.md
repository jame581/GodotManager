...
- **Checksum verification**: installs from auto-built URLs are checked against `SHA512-SUMS.txt` published on `godotengine/godot-builds`. A mismatch aborts the install and deletes the downloaded archive. Verification is skipped silently (not an error) for a custom `--url` or a local `--archive`, since there's no upstream release to check against. For an auto-built-URL install, if the checksums can't be fetched for another reason, godman continues without them and always prints a warning; pass `--verbose` to see the specific reason (a 404 because none were published, a network failure, etc.).
